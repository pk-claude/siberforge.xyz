...
function renderPipeline() {
  ...
  const tiles = [
    ...
    { label: 'Completions YoY', value: lc ? `${lc.value >= 0 ? '+' : ''}${fmt(lc.value, 1)}%` : '—',
    ...
    { label: 'New Home Sales YoY', value: lns ? `${lns.value >= 0 ? '+' : ''}${fmt(lns.value, 1)}%` : '—',
    ...
    { label: 'Existing Sales YoY', value: les ? `${les.value >= 0 ? '+' : ''}${fmt(les.value, 1)}%` : '—',
...
function renderAffordability() {
  ...
  const tiles = [
    ...
    { label: '15Y Mortgage', value: l15 ? `${fmt(l15.value, 2)}%` : '—',
      meta: 'Freddie Mac PMMS · weekly',
...
function renderStress() {
  ...
  const tiles = [
    ...
    { label: 'Construction Employment YoY', value: lce ? `${lce.value >= 0 ? '+' : ''}${fmt(lce.value, 1)}%` : '—',
    ...
    { label: 'Lumber PPI YoY', value: ll ? `${ll.value >= 0 ? '+' : ''}${fmt(ll.value, 1)}%` : '—',
      meta: 'monthly · WPU081',
